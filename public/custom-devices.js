(function(){
  ...
  function maxDevices(){
    const n=isReseller()?Number(state.me.max_device_limit||2000):2000;
  ...
  function updateEstimation(){
    ...
    if(!input||!out)return;
    if(!isReseller()){out.textContent="";return;}
    const devices=Number(input.value),max=maxDevices(),plan=selectedPlan();
    if(!Number.isInteger(devices)||devices<1||devices>max){
      out.textContent=`Enter a whole number from 1 to ${max} devices.`;return;
    }
    if(!plan){out.textContent="Select an active duration.";return;}
    const price=Number(plan.price)||0,total=price*devices;
    out.textContent=`Price: ₹${price.toLocaleString("en-IN")} / Device • Estimated deduction: ₹${total.toLocaleString("en-IN")}`;
  }
  function renderCreate(){
    ...
    $id("duration").innerHTML=durations.map(x=>{
      ...
      const label=isReseller()?`${esc(x)} — ₹${Number(p?.price||0).toLocaleString("en-IN")} / Device`:esc(x);
      return `<option value="${escapeAttr(x)}">${label}</option>`;
    }).join("");
  ...
  window.createKey=async function(){
    try{
      await refreshState();state.plans=await api("/api/plans");
      const duration=$id("duration").value,devices=Number($id("devices").value),max=maxDevices(),plan=selectedPlan();
